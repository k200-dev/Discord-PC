...
async def on_ready():
    ...
    await bot.change_presence(
        activity=discord.Activity(
            type=discord.ActivityType.listening, name="the flames of a PC"
        )
    )
...
async def _lock(ctx):
    ...
    for x in banned_processes:
        if x in output:
            await ctx.send(
                f"⚠️ **{username}** is currently running a banned process. **({x})**"
            )
            return
...
async def message(ctx, message_to_send: str):
    ...
    await ctx.send(
        content=f"💬 Sending dialogue box with **{message_to_send}** to **{username}s** PC."
    )
...
async def search(ctx, thing_to_search: str):
    ...
    for x in banned_words:
        if x in thing_to_search:
            ...
            return
    await ctx.send(
        content=f"🔍 Searching for **{thing_to_search}** on **{username}s** PC."
    )
...
@slash.slash(
    ...
    options=[
        create_option(
            name="quote", description="The quote to add", option_type=3, required=True
        )
    ],
)
...
# Create the add quote to book command
@slash.slash(
    name="serverping", description="Checks the ping and online status of a server"
)
